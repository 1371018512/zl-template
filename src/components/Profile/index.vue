<template>
  <div>
    <img
      v-popover:popover1
      :src="$store.getters['user/userDetail'].profile"
      ...
      <el-container>
        <el-header style="padding: 0;height: 30px;">
          <el-row type="flex" class="row-bg" justify="space-between">
            <el-col :span="15" style="line-height: 30px;font-size: 10px;">{{
              $store.getters["user/userDetail"].nickName
            }}</el-col>
            <el-col :span="8">
...
export default {
  ...
  methods: {
    goSelf() {
      this.$router.push({
        path: "/person/index/" + this.$store.getters["user/uId"],
      });
      /* setTimeout(() => {
					this.$router.go(0);
				},100) */
